...
\chapter*{Abstract}
A probabilistic linear dynamical system (pLDS) models the linear evolution of a latent space and an observed space. Both spaces are assumed to be corrupted by gaussian noise. In this work, we lay out a theoretical framework to model the parameters of a pLDS when we only have samples from the observed space. We employ the EM algorithm iteratively to estimate the latent samples and update the model parameters. We show that the E-step gives rise to the Kalman filter and Kalman smoother equations. We provide numerical examples to show the use of the Kalman filter and Kalman smoother equations. Finally, we present ideas to extend the assumptions of linearity considered in this work.
