...
In some scenarios, we may assume that the observed dataset is the result of a continuous system. In this case, we would have discrete observations resulting from a continuous system. We can write the pLDS that corresponds to this scenario as follows
\begin{align}
	\dot{\z} &= {\bf A}\z + \boldsymbol\varepsilon,\\
	\x &= {\bf C} \dot{\z} + \boldsymbol{\varphi}.
\end{align} 
Where $p(\boldsymbol{\varepsilon}) = \N(\boldsymbol{\varepsilon} \vert {\bf 0}, \boldsymbol\Sigma)$, and $p(\boldsymbol{\varphi}) = \N(\boldsymbol{\varphi} \vert {\bf 0}, \boldsymbol{\Gamma})$. It can be shown (\texttt{TODO: add reference}) that the Kalman filter equations take the form:
\begin{align}
	ds
\end{align}
