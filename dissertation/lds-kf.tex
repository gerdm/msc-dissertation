...
% Introduce the topic and research question (Why this topic?)
\section{Machine learning and dynamical systems}
In many machine learning models, we assume that the data used to train the models contain all of the information required to make a prediction. Examples of these models are the linear regression (and its variants), the logistic regression, and neural networks. This assumption extends to time-series models such as the ARMA, GARCH, and VAR models, on which the estimation of the next time-step is a function of previously-seen observations. In an $\text{VAR}(p)$ model, for example, the evolution of a $K$-dimensional vector $\z_n$ is taken to be a linear combination of the previous $K$ observations that preceded $\z_n$. That is,
\begin{equation}
	\z_{n} = \sum_{k=1}^K {\bf A}_k\z_{n-k} + \boldsymbol{\varepsilon}_n, \label{eq:var-model}
\end{equation}
where $\boldsymbol{\varepsilon}_n$ is some noise term.

If the data we want to model is taken to be noise-less. We might pose the problem in terms of a dynamical system. A dynamical system model captures the times evolution of a system (\cite{strogatz}). A discrete-time linear dynamical system is a $K$-dimensional vector $\z_n$ which evolves according to a real-valued matrix ${\bf A}\in\R^{K\times K}$ in the form

\begin{equation}
	\z_{n+1} = {\bf A}\z_n \label{eq:linear-dynamical-system}
\end{equation}
...
\x_{18}$. If compare equations \eqref{eq:var-model} and \eqref{eq:linear-dynamical-system}, we observe that a linear dynamical system can be seen as a noise-less $\text{VAR}(1)$ model.

% Why do we care that a VAR(1) model can be seen as a linear dynamical system?
...
\begin{align}
	\z_{n+1} &= {\bf A} \z_{n}, \label{eq:noisless-dynamical-system-1}\\
	\x_{n+1} &= {\bf C} \z_{n+1} \label{eq:noisless-dynamical-system-2}.
\end{align}
...
\begin{figure}[h!]
	\hfill
	\subfigure[Actual spacecraft position]{\includegraphics[width=6cm]{../figures/spacecraft-actual}}
	\hfill
	\subfigure[Observed spacecraft position]{\includegraphics[width=6cm]{../figures/spacecraft-observed}}
	\hfill
...
Throughout this work every random variable is taken to be real-valued and every random vector is taken to be a real-valued column vector. To be specific, for every $n$, we denote a latent variable as $\z_n \in \R^K$ and an observed variable as $\x_n \in \R^M$, where $K \geq 1$ is the dimensionality of the latent space and $M \geq 1$ is the dimensionality of the observed space. To reduce clutter in the notation, every integral  symbol in this work assumes a definite integral over the real numbers with dimension specified by the differential term. Furthermore, we denote by $p(x)$ the \textit{density} of a random variable $X$ evaluated at $x$.

We begin Chapter 2 outlining our model of interest, the probabilistic linear dynamical system (pLDS), and presenting the tools that we use throughout this work. Namely, the framework of the probabilistic graphical models framework and the Expectation-Maximisation (EM) algorithm. In Chapter 3, we make use of probabilistic graphical models and the EM algorithm to  derive a parameter-estimation procedure of the pLDS. We show that the E-step in the EM algorithm gives rise to the Kalman filter and Kalman smoother equations. Furthermore, we show that under a pLDS, the M-step results in a closed-form solution which can be readily evaluated at each iteration. In Chapter 4, we present two numerical applications on the use of Kalman filtering. Finally, in chapter 5, we draw some conclusions about the use and the applications of the model. Furthermore, we provide references to works that expand the ideas presented in this dissertation.
...
\begin{proof}
	...
	\begin{figure}[h!]
		\centering
		\resizebox{\columnwidth}{!}{\input{../figures/lds-skeleton.tikz}}
	...
	\begin{figure}[h!]
		\centering
		\resizebox{\columnwidth}{!}{\input{../figures/graphical-models/gm1.tikz}}
	...
	\begin{figure}[h!]
		\centering
		\resizebox{\columnwidth}{!}{\input{../figures/graphical-models/gm2.tikz}}
	...
	\begin{figure}[h!]
		\centering
		\resizebox{\columnwidth}{!}{\input{../figures/graphical-models/gm3.tikz}}
	...
	\begin{figure}[h!]
		\centering
		\resizebox{\columnwidth}{!}{\input{../figures/graphical-models/gm4.tikz}}
	...
	\begin{figure}[h!]
		\centering
		\resizebox{\columnwidth}{!}{\input{../figures/graphical-models/gm5.tikz}}
...
\end{equation}
To achieve the E-step, we require to take the derivative of $Q(\boldsymbol{\theta}, \boldsymbol{\theta}^\text{old})$ with respect to each term in $\boldsymbol\theta = \{{\bf A}, {\bf C}, \boldsymbol\Gamma, \boldsymbol\Sigma, \boldsymbol\mu_0, {\bf V}_0\}$. We use the results found in \cite{matrix-cookbook} to take derivatives with respect to matrices. To derive the following terms, note that we will be working with a function of the form $f:\mathbb{\R^{M \times M} \to \R}$. Then, a derivative $\bf F$ of $f({\bf X})$ for ${\bf X}\in\R^{M\times M}$ is defined as

\begin{equation}
	{\bf F}_{ij} = \frac{\partial}{\partial {\bf X}_{ij}} f({\bf X}).
\end{equation}

For the M-step, we consider the function $Q(\boldsymbol\theta, {\boldsymbol\theta}^\text{old})$, which takes matrices as inputs and returns a real number. We divide the task of finding the updating equations in the following propositions:
...
Consider the following example based on \cite{pml2Book}, and \cite{koller2009}. We want to track a set of fireworks in the air. We have multiple noisy observations of the fireworks and we seek to recover the \textit{true} latent path (see Figure \ref{fig:fireworks-latent-observed}). We model the latent space of each firework in a 4-dimensional space: the first two dimensions relate to the position of the firework, while the last two dimensions relate to the velocity of the firework. We capture the latent dynamics in the following matrix $\bf A$:
\begin{equation}
	{\bf A} = \begin{bmatrix}
		1 & 0 & \Delta & 0 \\
		0 & 1 & 0 & \Delta \\
...
with $\Delta \in \R^+$. Next, the observed space is taken to be a linear projection of the latent space onto $\R^2$ via 
\begin{equation}
	{\bf C} = \begin{bmatrix}
		1 & 0 & 0 & 0\\
		0 & 1 & 0 & 0\\
...
In this work, we lay out a theoretical framework for modelling pLDS. We showed that the pLDS model can be seen as a dynamical system model with gaussian noise. Furthermore, we introduced the framework of probabilistic graphical models. We showed that translating the pLDS as dynamical system model into a probabilistic graphical model allows us to use the technique of d-separation to find factorisations between nodes in the graph. We showed multiple factorisations from the pLDS that can be derived using d-separation.

 In addition, we presented the Expectation Maximisation (EM) algorithm. The EM algorithm is used to estimate the parameters of a model with latent variables. We showed that the EM algorithm is divided into two steps: the E-step, which computes the posterior distribution of a latent variable conditioned on the observed data-points up to the time of estimation, i.e., it estimates $p(\z_n \vert \x_1, \ldots, \x_n)$; and the M-step, which maximises the model parameters with respect the expected value of the complete-data log-likelihood conditional on the posterior distribution found in the E-step.
 
 Next, we showed that the computation of the E-step in the EM algorithm applied to the pLDS can be simplified by considering how the nodes in the complete-dataset factorise. As a result, we showed that this step leads to a set of equations known as Kalman filters: a technique used to estimate the latent dataset on-line. Furthermore, if we run the Kalman filter algorithm until we have observe all data-points, we showed that we obtain the Kalman smoother equations. We exemplified the Kalman filter and Kalman smoother equations in two examples.

It is important to consider that limitation of a pLDS occurs when either the latent dynamics or the observed dynamics are not linear. In this scenario, we can make use of the extended Kalman filter (EKF) or the unscented Kalman filter (UKF) (see \cite{pml2Book}) for a first-order approximation of the latent space while retaining the equations provided by theorem \ref{theorem:kalman-filter}. Both EKF and UKF models assume that the underlying nonlinear dynamics are known. If the latent and observed dynamics are nonlinear and unknown, \cite{krishnan2015deep} provide a method to estimate the  dynamics using deep neural networks. Finally, if the observed variables are assumed to affect one another, \cite{deeprl} propose a modification of the pLDS graphical model to consider additional factors that affect the observed space.
