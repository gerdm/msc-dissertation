...
In this work, we derived a model that is capable of capturing latent dynamics of a linear system and provide a set of equations that efficiently estimate the latent space, and provide updates for the parameters of the system. A big limitation of the model happens whenever either the latent dynamics or the observed dynamics do not behave linearly. In this scenario, we can make use of the Extended Kalman Filter (EKF) or the Unscented Kalman Filter (UKF) (see \cite{pml2Book}) for a first-order approximation of the latent space while retaining the equations provided by theorem \ref{theorem:kalman-filter}. Both EKF and UKF models assume that the the underlying nonlinear dynamics are known. If the latent and observed dynamics are nonlinear and unknown, \cite{krishnan2015deep} provide a method to estimate the dynamics using deep neural networks. Finally, is we assume there is an additional input in the system, \cite{deeprl} propose a modification of the graphical model presented in this work to consider additional factors that affect the observed space.
