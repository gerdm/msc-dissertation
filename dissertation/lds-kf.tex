...
\subsection{Example 2: continuous systems}
In some scenarios, we may assume that the observed dataset is the result of a continuous system. Hence, we would have discrete observations of a continuous system. We can modify our assumptions in the system as follows
\begin{align}
	\dot{\z} &= {\bf A}\z + \boldsymbol\varepsilon\\
	\x &= {\bf C} \dot{\z} + \boldsymbol{\varphi}.
\end{align} 
Where $p(\boldsymbol{\varepsilon}) = \N({\bf 0}, \boldsymbol\Sigma)$, and $p(\boldsymbol{\varphi}) = \N({\bf 0}, \boldsymbol{\Gamma})$.
